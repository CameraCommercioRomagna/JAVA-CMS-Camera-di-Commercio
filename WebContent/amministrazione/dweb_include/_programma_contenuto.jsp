<form name="forminsert" enctype="multipart/form-data" method="post" action="/amministrazione/esegui_programma.jsp" class="form-control-cciaa-sm">
	...
	<div class="form-group row">
		...
		<%	}%>
		<%	id_modal = "modalEliminaImmaginePuntoProgramma"+punto.id_punto;
			if(punto.isInserted() && punto.img_path!=null && !punto.img_path.equals("")){%>
				...
				<!--%@include file="/amministrazione/componenti/_modal_elimina_immagine.jsp" %--> <%//inserito a fondo pagina per non innestare una form dentro altra form %>
		<%	}%>
		</div>
	</div>
	<div class="form-group row">
	...
	<div class="form-group row">
		...
		<div class="col-sm-10">
			...
			<button type="reset" style="width:200px;" class="btn btn-secondary" data-toggle="collapse" data-parent="#<%=(punto.isInserted() ? "programma" : "nuovo_programma")%>" href="#programma<%=(punto.isInserted() ? punto.id_punto : "00")%>" aria-expanded="false" aria-controls="programma<%=(punto.isInserted() ? punto.id_punto : "00")%>">Annulla</button>
	...
	<%	if(punto.isInserted()){%>	
		<input type="Hidden" name="id_punto" value="<%=punto.id_punto %>" />
	<%	}%>
...
<%	id_modal = "modalEliminaImmaginePuntoProgramma"+punto.id_punto;
	if(punto.isInserted() && punto.img_path!=null && !punto.img_path.equals("")){
		doc_modal = pagina;
		form_action = "/amministrazione/esegui_programma.jsp";
		form_add_input_hidden = "<input type=\"Hidden\" name=\"id_punto\" value=\"" + punto.id_punto + "\" />";
